...
| Wi-Fi | ❌ 不可用 | 能连上，但**每次开机约 7 分钟后固件崩溃**，根因已定性且无法移植修复 → 建议直接关掉。见 [docs/项目总览.md §4](docs/项目总览.md) |
| 蜂窝（手机卡） | ⚠️ 未通，**但值得一试** | 数据面被 IPA 卡住（`ipa.ko` 一加载即整机复位，已黑名单）。但 IPA v3.1 **主线原生支持**、本机 IPA 节点接线完整、**本机 Android 下数据面跑通过** ⇒ 2026-09-25 改判为可尝试（估 1/3~1/2）。见 [docs/蜂窝可行性评估.md](docs/蜂窝可行性评估.md) |
| 摄像头 / NFC / 指纹 / 传感器 | ❓ | 未验证 |
| 显示 | ⚠️ | 下半屏重叠（上游已知缺陷） |
...
| 我想… | 看这里 |
|---|---|
| **先看个全景** —— 项目地址、设备信息、能力清单、Wi-Fi 与蜂窝为什么修不好 | [docs/项目总览.md](docs/项目总览.md) |
| **第一次上手** —— 连上设备、日常操作、命令速查 | [docs/使用说明.md](docs/使用说明.md) |
| **出问题了** —— 17 个故障的现象 / 判据 / 根因 / 修法 + 诊断方法论 | [docs/修复记录.md](docs/修复记录.md) |
| **备份与恢复** —— 四层备份策略、恢复步骤、哪些操作会变砖 | [docs/备份与恢复.md](docs/备份与恢复.md) |
| **🔴 要动 boot 镜像** —— 签名规则、本地验签、三重闸门、黑屏后三条恢复通道 | [docs/boot镜像签名与恢复.md](docs/boot镜像签名与恢复.md) |
| **卡死时怎么留证据** —— 三层日志与取证体系 | [docs/日志与取证.md](docs/日志与取证.md) |
| **硬件到底能用什么** —— Wi-Fi / 蜂窝 / 蓝牙 / 音频的定量结论 | [docs/硬件现状.md](docs/硬件现状.md) |
| **Wi-Fi / 手机卡还有没有办法** —— 可选方案的评审、三条零成本前置实验、成本与风险重估 | [docs/WiFi与蜂窝修复方案.md](docs/WiFi与蜂窝修复方案.md) |
...
└── docs/
    ...
    ├── boot镜像签名与恢复.md   🔴 改 boot 前必读：改了就必须重签，否则黑屏；含本地验签与三条恢复通道
    ├── WiFi与蜂窝修复方案.md   Wi-Fi 软解 / 蜂窝绕行的方案评审与实施清单（含"只换 .ko，不刷 boot"）
    ├── 使用说明.md             日常怎么用、怎么连、命令速查
    ├── 修复记录.md             17 个故障的现象/判据/根因/修法 + 诊断方法论
...
> ✅ **每台 PC 只需要配一次。** 真正生效的是持久化的 `192.168.137.1`（宿主侧 ICS 网卡）
> 加一条 WinNAT（`z17s-usb`），两者都不随 USB 重新枚举消失 —— 判据是 `Get-NetNat` 里还列着它。
> 只有当「手机能 ping 通 PC、却出不了公网」时才需要重跑，**先查 `Get-NetNat` 再动手**。
> Windows 显示「未识别的网络」是正常现象，不是故障。
...
Wi-Fi 与关机语义**已定性，别再重复投入**；**蜂窝 2026-09-25 改判为"值得一试"**（见 [docs/蜂窝可行性评估.md](docs/蜂窝可行性评估.md)）。
完整分析见 [docs/项目总览.md](docs/项目总览.md)。

- **Wi-Fi 每次开机活不过约 7 分钟**，崩溃后只能重启恢复。根因是 `mac80211.ko` 里的自研钩子
  `key_hw_accel` 强制硬件解密，而 `cipher=0xfac04` 就是标准 WPA2 —— 几乎所有现代路由器都踩中；
  崩的是 **modem（Q6）里的 Wi-Fi 固件自己 assert**。主线 6.17 与一加 7T（同款 WCN3990）同样必崩，
  **没有可移植的修复**。处置：`nmcli radio wifi off` 直接关掉止损（负载 1.9 → 0.28）。
- **蜂窝（手机卡）上不了网**，但**不是"主线不支持蜂窝"**：modem 是活的、SIM 也认得出来，
  被卡住的是**数据面** —— MSM8998 的 modem 数据**必须经过 IPA 加速器**，而 `ipa.ko` 一加载就
  把本机重启（`# IPA resets this NX595J with the current 6.12 device tree`），已进黑名单。
  没有 IPA 就没有 `rmnet_data0`。**换 OpenWrt 也救不了**（同一套内核驱动，见总览 §5.4）。
  ⚠️ **2026-09-25 改判**：上面的判断成立，但"无解"的推论不成立 —— 主线**原生支持** `qcom,msm8998-ipa`（IPA v3.1）、
  本机设备树 IPA 节点**接线完整**（固件也在设备上）、modem 已在 QRTR 上广播 **IPA control service**、
  且**本机 Android 下 IPA 数据面初始化成功过**（原始日志仍在）。⇒ 值得一试，见 [docs/蜂窝可行性评估.md](docs/蜂窝可行性评估.md)。
- **关机"慢"的真相**：一次干净关机其实只要 **约 11 秒**，其中 **10.7 秒在等 `docker.service` 停** ——
  关机前 `systemctl stop docker` 能压到 1~2 秒。另外按电源键 **点一下就该松手**
  （logind 实测 `HandlePowerKeyLongPress=ignore`，"按多久"软件层完全不管；
  按住等于白送时间给 PMIC 的 10~12 秒长按复位逻辑，会出现"关完 3 秒又自己开机"）。
