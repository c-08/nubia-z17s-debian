...
| 蜂窝（手机卡） | ⚠️ 未通，**但值得一试** | 数据面被 IPA 卡住。但 IPA v3.1 **主线原生支持**、本机 IPA 节点接线完整、**本机 Android 下数据面跑通过** ⇒ 改判为可尝试（估 1/3~1/2）；`ipa.ko` 已备好待上机做 G1。见 [docs/蜂窝可行性评估.md](docs/蜂窝可行性评估.md) |
| 摄像头 / NFC / 指纹 / 传感器 | ❓ | 未验证 |
| 显示 | ⚠️ | 下半屏重叠（上游已知缺陷） |

**结论：这是一台"USB 连着用"的机器。** 网络走 USB，屏幕当辅助，服务的稳定性不依赖 Wi-Fi。

> 🔴 **刷机唯一通道 = TWRP（音量上+电源）+ `adb push` + `dd`**。
> 本机 fastboot **不能刷分区**（实测 `flash`/`download` 全是 `unknown command`，且 `reboot recovery` 回 `OKAY` 是假信号）。
